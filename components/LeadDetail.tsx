    ...
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-3">
              ...
              <span className="text-lg font-bold text-[#8B5CF6]">{lead.opportunityScore}</span>
            </div>
        ...
        <div className="flex gap-2">
          {lead.whatsapp && (
            <a href={`https://wa.me/${lead.whatsapp}`} target="_blank" className="flex items-center gap-2 bg-[#16161A] hover:bg-[#25D366]/10 text-[#25D366] px-3 py-2 rounded-lg text-sm transition-colors border border-[#25D366]/20">
              <MessageCircle size={15} />WhatsApp
            </a>
          )}
          {!editing ? (
            <button onClick={() => setEditing(true)} className="flex items-center gap-2 bg-[#16161A] hover:bg-[rgba(139,92,246,0.1)] text-white px-3 py-2 rounded-lg text-sm transition-colors border border-[rgba(139,92,246,0.1)]">
              <Edit2 size={15} />Editar
            </button>
          ) : (
            <div className="flex gap-2">
              <button onClick={() => setEditing(false)} className="flex items-center gap-2 bg-[#16161A] text-[#6B7280] px-3 py-2 rounded-lg text-sm">
                <X size={15} />Cancelar
              </button>
              <button onClick={handleSave} disabled={saving} className="flex items-center gap-2 bg-[#8B5CF6] hover:bg-[#A78BFA] text-white px-3 py-2 rounded-lg text-sm">
          ...
          {PIPELINE_STAGES.map((s, i) => (
            <div key={s.id} className="flex items-center gap-2 flex-shrink-0">
              <button
                ...
                style={lead.pipelineStatus === s.id ? { background: s.color, boxShadow: `0 0 12px ${s.color}40` } : { background: '#16161A' }}
              >
                {s.label}
              </button>
      ...
      <div className="grid grid-cols-3 gap-6">
        ...
        <div className="col-span-2 space-y-4">
          ...
          <div className="card-dark p-5">
            <h3 className="text-sm font-semibold text-[#8B5CF6] uppercase tracking-wider mb-4">Presença Digital</h3>
                ...
                return (
                  <div key={key} className={`p-3 rounded-lg border ${isGood ? 'border-[rgba(16,185,129,0.2)] bg-[rgba(16,185,129,0.05)]' : 'border-[rgba(239,68,68,0.2)] bg-[rgba(239,68,68,0.05)]'}`}>
                    <div className="flex items-center justify-between">
                      ...
                      {editing ? (
                        <input type="checkbox" checked={(form as any)[key]} onChange={e => setForm(f => ({ ...f, [key]: e.target.checked }))} className="w-4 h-4 accent-[#8B5CF6]" />
                      ) : (
          ...
          {/* Notes */}
          <div className="card-dark p-5">
            <h3 className="text-sm font-semibold text-[#8B5CF6] uppercase tracking-wider mb-3">Observações</h3>
            {editing ? (
              <textarea
              ...
              <p className="text-sm text-[#A1A1AA]">{lead.observacaoPerfil || <span className="text-[#4B5563]">Sem observações</span>}</p>
            )}
          </div>

          {/* Activities */}
          <div className="card-dark p-5">
            <h3 className="text-sm font-semibold text-[#8B5CF6] uppercase tracking-wider mb-4 flex items-center gap-2">
        ...
        <div className="space-y-4">
          <div className="card-dark p-5 space-y-4">
            <h3 className="text-sm font-semibold text-[#8B5CF6] uppercase tracking-wider">Contacto</h3>
            {lead.telefone && <div className="flex items-center gap-2 text-sm text-[#A1A1AA]"><Phone size={13} />{lead.telefone}</div>}
            {lead.email && <div className="flex items-center gap-2 text-sm text-[#A1A1AA]"><Mail size={13} />{lead.email}</div>}
          </div>

          <div className="card-dark p-5 space-y-3">
            <h3 className="text-sm font-semibold text-[#8B5CF6] uppercase tracking-wider">Planos</h3>
            <div>
            ...
            <div>
              ...
              <div className="text-sm text-[#8B5CF6]">{lead.planoAlvoUpgrade || '—'}</div>
            </div>
          </div>

          <div className="card-dark p-5 space-y-3">
            <h3 className="text-sm font-semibold text-[#8B5CF6] uppercase tracking-wider">Follow-Ups</h3>
            {lead.followUps.slice(0, 3).map(f => (
              <div key={f.id} className={`text-xs p-2 rounded-lg ${f.enviado ? 'text-[#4B5563]' : 'text-[#A1A1AA] bg-[#16161A]'}`}>
          ...
          <div className="card-dark p-5">
            <h3 className="text-sm font-semibold text-[#8B5CF6] uppercase tracking-wider mb-3">Propostas</h3>
            {lead.proposals.map(p => (
              <div key={p.id} className="flex items-center justify-between text-sm py-1.5">
                <span className="text-[#A1A1AA] truncate">{p.titulo}</span>
            ...
            <Link href={`/propostas/nova?leadId=${lead.id}`} className="mt-3 w-full flex items-center justify-center gap-2 bg-[rgba(139,92,246,0.1)] hover:bg-[rgba(139,92,246,0.2)] text-[#8B5CF6] text-xs py-2 rounded-lg transition-colors">
