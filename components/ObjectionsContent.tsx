  ...
  return (
    <div className="space-y-6">
      ...
      {categories.map(cat => {
        ...
        return (
          <div key={cat}>
            <div className="text-xs font-semibold text-[#FF6A00] uppercase tracking-wider mb-3">{cat}</div>
            <div className="space-y-3">
              {catObs.map(o => (
                <div key={o.id} className="card-dark p-5 space-y-3">
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex items-start gap-3">
                      <MessageSquareQuote size={16} className="text-[#FF6A00] mt-0.5 flex-shrink-0" />
                      <p className="text-white font-medium text-sm">{o.objecao}</p>
                    </div>
                  </div>
                  <div className="bg-[#1A1A1F] rounded-lg p-4 relative group">
                    <p className="text-sm text-[#9CA3AF] leading-relaxed pr-8">{o.resposta}</p>
                    <button
                      onClick={() => copy(o.resposta, o.id)}
                      className="absolute top-3 right-3 text-[#4B5563] hover:text-[#FF6A00] opacity-0 group-hover:opacity-100 transition-all"
                    >
